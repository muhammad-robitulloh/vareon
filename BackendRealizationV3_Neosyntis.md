...
*   **Database Model (`database.py` / `models.py`)**:
    *   [x] Define SQLAlchemy model for `Workflow` with fields: `id` (UUID), `name` (string), `description` (optional string), `status` (enum: `pending`, `running`, `completed`, `failed`), `steps` (JSON/array of strings), `created_at` (datetime), `updated_at` (datetime), `owner_id` (foreign key to User).
    *   [ ] Consider a separate `WorkflowStep` model if steps require more complex attributes (e.g., status per step, parameters).
*   **Pydantic Schemas (`schemas.py`)**:
    *   [x] Update `Workflow` schema to reflect database model.
    *   [x] Create `WorkflowCreate` schema for new workflow creation.
    *   [x] Create `WorkflowUpdate` schema for updating existing workflows.
*   **CRUD API Endpoints (`main.py`)**:
    *   [x] `POST /api/neosyntis/workflows`: Create a new workflow.
        *   Input: `WorkflowCreate` schema.
        *   Output: Created `Workflow` object.
        *   Logic: Store in DB, assign owner.
    *   [x] `GET /api/neosyntis/workflows`: List all workflows (with optional filtering/pagination).
        *   Output: List of `Workflow` objects.
        *   Logic: Fetch from DB.
    *   [x] `GET /api/neosyntis/workflows/{workflow_id}`: Get a specific workflow by ID.
        *   Output: `Workflow` object.
        *   Logic: Fetch from DB.
    *   [x] `PUT /api/neosyntis/workflows/{workflow_id}`: Update an existing workflow.
        *   Input: `WorkflowUpdate` schema.
        *   Output: Updated `Workflow` object.
        *   Logic: Update in DB.
    *   [x] `DELETE /api/neosyntis/workflows/{workflow_id}`: Delete a workflow.
        *   Logic: Delete from DB.
    *   [x] `POST /api/neosyntis/workflows/{workflow_id}/trigger`: Trigger workflow execution.
        *   Logic: Update workflow status to `running`, initiate background task for execution (placeholder for now).
    *   [x] `GET /api/neosyntis/workflows/{workflow_id}/status`: Get workflow execution status.
...
#### 2. Datasets
*   **Database Model (`database.py` / `models.py`)**:
    *   [x] Define SQLAlchemy model for `Dataset` with fields: `id` (UUID), `name` (string), `description` (optional string), `file_path` (string, path to stored file), `size_bytes` (integer), `format` (string), `uploaded_at` (datetime), `owner_id` (foreign key to User).
    *   [ ] Consider fields for metadata, tags, access control.
*   **Pydantic Schemas (`schemas.py`)**:
    *   [x] Update `Dataset` schema to reflect database model.
    *   [x] Create `DatasetCreate` schema for new dataset creation (might include file upload details).
    *   [x] Create `DatasetUpdate` schema for updating dataset metadata.
*   **CRUD API Endpoints (`main.py`)**:
    *   [x] `POST /api/neosyntis/datasets/upload`: Upload a new dataset.
    ...
    *   [x] `GET /api/neosyntis/datasets`: List all datasets (with optional filtering/pagination).
        *   Output: List of `Dataset` objects.
        *   Logic: Fetch from DB.
    *   [x] `GET /api/neosyntis/datasets/{dataset_id}`: Get a specific dataset by ID.
        *   Output: `Dataset` object.
        *   Logic: Fetch from DB.
    *   [x] `PUT /api/neosyntis/datasets/{dataset_id}`: Update dataset metadata.
        *   Input: `DatasetUpdate` schema.
        *   Output: Updated `Dataset` object.
        *   Logic: Update metadata in DB.
    *   [x] `DELETE /api/neosyntis/datasets/{dataset_id}`: Delete a dataset.
        *   Logic: Delete file from storage, delete metadata from DB.
    *   [x] `GET /api/neosyntis/datasets/{dataset_id}/download`: Download a dataset.
        *   Output: File stream.
        *   Logic: Retrieve file from storage and stream it.
*   **Storage Logic (New module, e.g., `storage_service.py`)**:
    *   [x] Functions for saving, retrieving, and deleting files from a configured storage location (e.g., local filesystem, S3, etc.).
...
#### 1. Telemetry
*   **Database Model (`database.py` / `models.py`)**:
    *   [x] Define `TelemetryData` model (e.g., `timestamp`, `metric_name`, `value`, `workflow_id`/`dataset_id`/`device_id` for context).
*   **API Endpoints (`main.py`)**:
    *   [x] `GET /api/neosyntis/telemetry`: Retrieve aggregated telemetry data (e.g., for charts).
        *   Input: Query parameters for time range, metric type, aggregation level.
        *   Output: Aggregated telemetry data.
    *   [x] `POST /api/neosyntis/telemetry/ingest`: Endpoint for internal services/agents to push telemetry data.
...
*   **API Endpoint (`main.py`)**:
    *   [x] `GET /api/neosyntis/search`: Search across Neosyntis entities (workflows, datasets, models).
...
*   **Database Model (`database.py` / `models.py`)**:
    *   [x] Define `MLModel` model (e.g., `id`, `name`, `version`, `status`, `path_to_model_artifact`, `training_dataset_id`, `deployed_at`, `owner_id`).
    *   [x] Define `TrainingJob` model (e.g., `id`, `model_id`, `status`, `start_time`, `end_time`, `metrics`).
*   **Pydantic Schemas (`schemas.py`)**:
    *   [x] `MLModel`, `MLModelCreate`, `MLModelUpdate`.
    *   [x] `TrainingJob`, `TrainingJobCreate`.
*   **API Endpoints (`main.py`)**:
    *   [x] `POST /api/neosyntis/models`: Upload/register a new ML model.
    *   [x] `GET /api/neosyntis/models`: List all ML models.
    *   [x] `GET /api/neosyntis/models/{model_id}`: Get model details.
    *   [x] `POST /api/neosyntis/models/{model_id}/deploy`: Deploy a model to an inference endpoint.
    *   [x] `POST /api/neosyntis/models/{model_id}/train`: Start a training job for a model.
    *   [x] `GET /api/neosyntis/training-jobs`: List training jobs.
    *   [x] `GET /api/neosyntis/training-jobs/{job_id}`: Get training job details.
...
*   **Enhancement**: Ensure these endpoints are backed by a database and not just in-memory storage.
    *   [x] **Database Model**: Define `RoutingRule` model in `database.py` (if not already done).
    *   [x] **API Endpoints**: Modify existing endpoints to interact with the database.
