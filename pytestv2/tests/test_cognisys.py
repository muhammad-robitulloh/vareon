...
def admin_auth_headers_fixture(client, admin_user, mocker: MockerFixture):
    # Mock get_user_by_username_or_email to return the admin_user directly
    mocker.patch("server_python.database.get_user_by_username_or_email", return_value=admin_user)
    # Mock verify_password to always return True
    mocker.patch("server_python.auth.verify_password", return_value=True)
...
def test_create_llm_provider(client, admin_auth_headers, override_admin_permission_checker, override_get_current_admin_user):
    response = client.post(
...
def test_update_llm_provider(client, admin_auth_headers, db_session, override_admin_permission_checker, override_get_current_admin_user):
    provider = LLMProvider(
...
def test_delete_llm_provider(client, admin_auth_headers, db_session, override_admin_permission_checker, override_get_current_admin_user):
    provider = LLMProvider(
...
def test_test_llm_provider_connection(client, auth_headers, db_session, mocker: MockerFixture):
    # Mock httpx.AsyncClient().get to simulate a successful connection
    mock_response = mocker.Mock()
    mock_response.status_code = 200
    mock_response.raise_for_status.return_value = None # No exception
    mock_response.json.return_value = {"message": "Connection successful", "status": "success"}
...
def test_create_llm_model(client, admin_auth_headers, db_session, override_admin_permission_checker, override_get_current_admin_user):
    provider = LLMProvider(
...
def test_update_llm_model(client, admin_auth_headers, db_session, override_admin_permission_checker, override_get_current_admin_user):
    provider = LLMProvider(
...
def test_delete_llm_model(client, admin_auth_headers, db_session, override_admin_permission_checker, override_get_current_admin_user):
    provider = LLMProvider(
...
def test_chat_with_cognisys_general_query(client, auth_headers, db_session, test_user, mocker: MockerFixture):
    # Mock the call_llm_api function (this is still needed if process_chat_request calls it)
    mocker.patch(
        "server_python.cognisys.llm_interaction.call_llm_api",
        return_value={
            "message": {"content": "Mocked response for general query"}, # Ensure 'message' key is present
            "prompt_tokens": 10,
    ...
    # Mock process_chat_request to return a successful response
    mocker.patch(
        "server_python.cognisys.llm_interaction.process_chat_request",
        return_value={
            "response": "Mocked response for general query",
            "model_used": "google/gemini-pro",
            "visualization_data": {"nodes": [], "edges": []} # Provide dummy visualization data
        }
    )

    # Ensure a default model exists for fallback (this is still good practice)
    provider = LLMProvider(
...
def test_detect_intent_endpoint(client, auth_headers, db_session, test_user, mocker: MockerFixture):
    # 1. Create a test LLM Provider
    provider = LLMProvider(
        id="intent_provider", name="IntentProvider", base_url="http://intent.com",
        api_key_encrypted=encrypt_api_key("intentkey"), enabled=True, organization_id=None
    )
    db_session.add(provider)
    db_session.commit()
    db_session.refresh(provider)

    # 2. Create an LLM Model with 'intent_detection' role
    intent_model = LLMModel(
        id="intent_model", provider_id=provider.id, model_name="intent-detector", type="chat",
        is_active=True, reasoning=False, role="intent_detection", max_tokens=1000, cost_per_token=0.001
    )
    db_session.add(intent_model)
    db_session.commit()
    db_session.refresh(intent_model)

    # 3. Mock the call_llm_api function to return a predefined JSON response
    mocker.patch(
        "server_python.cognisys.llm_interaction.call_llm_api",
        return_value={
            "message": {"content": '{"intent": "shell_command", "confidence": 0.9, "reasoning": "User asked to list files."}'},
            "prompt_tokens": 50,
            "completion_tokens": 20,
            "total_tokens": 70,
            "model_used": "intent-detector"
        }
    )

    # 4. Call the /api/cognisys/detect-intent endpoint
    response = client.post(
        "/api/cognisys/detect-intent",
        headers=auth_headers,
        json={"prompt": "List all files in the current directory."}
    )

    # 5. Assert the response
    assert response.status_code == 200
    assert response.json()["intent"] == "shell_command"
