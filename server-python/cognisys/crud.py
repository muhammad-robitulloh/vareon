...
from database import LLMProvider, LLMModel, RoutingRule # Changed from ..database
...
def create_routing_rule(db: Session, rule: RoutingRuleCreate):
    db_rule = RoutingRule(
        id=str(uuid.uuid4()),
        owner_id=rule.owner_id, # Assuming owner_id is passed in the schema
        name=rule.name,
...
def update_routing_rule(db: Session, rule_id: str, rule: RoutingRuleUpdate):
    db_rule = get_routing_rule(db, rule_id)
    if db_rule:
        if rule.owner_id is not None:
            db_rule.owner_id = rule.owner_id
